...
export default function DateRangeSelector(props: Props) {
  ...
  const presetDateValues = () => {
    ...
    const selectDateRange: DateRangeIntervals[] = [
      ...
      { label: "Last 7 Days", range: [moment().subtract(7, "day").startOf("day").toDate(), toDate] },
      { label: "Last 14 Days", range: [moment().subtract(14, "day").startOf("day").toDate(), toDate] },
      { label: "Last 30 Days", range: [moment().subtract(30, "day").startOf("day").toDate(), toDate] },
      { label: "Last 90 Days", range: [moment().subtract(90, "day").startOf("day").toDate(), toDate] },
      { label: "Last Year", range: [moment().startOf("year").toDate(), toDate] },
    ];

    return selectDateRange.map((value: DateRangeIntervals, index: number) => {
      return <Button key={index} onClick={() => setRange(value.range)} size="medium" type="just_text">
        {value.label}
      </Button>;
    });
