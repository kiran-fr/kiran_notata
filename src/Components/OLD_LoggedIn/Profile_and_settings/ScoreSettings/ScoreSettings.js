...
import { color3_bg, color1_bg, color1, color3, color2 } from '../../Colors.module.css';
...
const Options = ({d, i, o, set, settings, field, prefix}) => {
  ...
  return (
    <div className={options_class}>
      {
        d.map((op, u) => {
          let hit = (settings[`${prefix}_${field}`] || []).find(ff => ff.key === op.val);
                ...
                <div>
                  {
                    [-1, 0, 1, 2, 3, 4, 5].map((iv, i) => (
                      <div
                        key={`dd-${i}-${o}-${u}-iv-${iv}`}
                        className={classnames(option_input, currentVal === iv ? color3_bg : option_not_selected)}
                        onClick={() => {
                          set({ key: op.val, val: iv, field: `${prefix}_${field}`})
                        }}
                      >{iv}</div>
                    ))
                  }
...

              {
                eData.map((d, i) => (
                  <div className={section} key={`ed-${i}`} >
                    <div className={classnames(section_title, color1_bg)}>{d.title}</div>
                    {
