...
export default function CommentBox({ connId }) {
  // Queries
  const { data, error } = useQuery(logGet, {
    variables: { connectionId: connId },
  });
  ...
  useEffect(() => {
    if (!data) return;
    let comm = data?.logGet?.filter(l => l.logType === "COMMENT");
    setComments(comm);
  }, [data]);

  // Comment Handeling Functions
  ...
  // Update Comment
  const updateComment = async (CommentID, INTVAL, index) => {
          ...
          <div className={styles.comment}>
            ...
            <div>
              <div className={styles.commentText}>
                {!hidden[index]
                  ? comment?.dataPairs?.length > 0 && comment?.dataPairs[0].val
                  : ""}
              </div>
