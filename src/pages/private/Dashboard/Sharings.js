...
export default function Sharings({ history }) {
  ...
  const [loadingMark, setLoadingMark] = useState({});
  ...
  if (userGetQuery.loading || groupsGetQuery.loading) {
    return <span />;
  }

  if (userGetQuery.error || groupsGetQuery.error) {
    return <span />;
  }

  if (!userGetQuery.data || !groupsGetQuery.data) {
    return <span />;
  }
  ...
  for (let group of groups) {
    let { name, members, startups } = group;
